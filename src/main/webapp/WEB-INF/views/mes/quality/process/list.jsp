    ...
    <div class="menu-nav">
        <div>
            <span class="btn btn-pill btn-sm btn-primary">
                <i class="fas fa-home"></i> <i class="fas fa-arrow-circle-right"></i> 품질관리 <i class="fas fa-arrow-circle-right"></i> 공정검사
            </span>
        </div>
                                ...
                                <table id="tblDataMaster" class="table table-hover table-bpurchaseed mb-3 table-form" style="margin-top: 35px;">
                                    ...
                                    <tbody>
                                        <tr role="row">
                                            <th>계획일</th>
                                            <td>
                                                <input type="hidden" id="frm_plan_cd" name="frm_plan_cd" value="" />
                                                <input type="hidden" id="frm_plan_proc_cd" name="frm_plan_proc_cd" value="" />
                                                <div id="frm_plan_dt" name="frm_plan_dt"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>작업지시번호</th>
                                            <td>
                                                <div id="frm_plan_no" name="frm_plan_no"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>거래처</th>
                                        ...
                                        <tr>
                                            <th>계획수량</th>
                                            <td>
                                                <div class="text-right" id="frm_plan_cnt" name="frm_plan_cnt"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>생산수량</th>
                                            <td>
                                                <div class="text-right" id="frm_plan_proc_cnt" name="frm_plan_proc_cnt"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>측정분류</th>
                                            <td>
                                                <select id="frm_plan_proc_quality_cd" name="frm_plan_proc_quality_cd" class="custom-select w-100" style="width: 80% !important;">
                                            ...
                                            <td>
                                                <div style="display: flex; flex-wrap: wrap;">
                                                    <div class="custom-control custom-radio my-3">
                                                        <input type="radio" checked=""  id="customRadio1" name="frm_plan_proc_quality_val" class="custom-control-input" value="Y">
                                                        <label class="custom-control-label" for="customRadio1">양호</label>
                                                    </div>
                                                    <div class="custom-control custom-radio my-3">
                                                        <input type="radio" id="customRadio2" name="frm_plan_proc_quality_val" class="custom-control-input" value="N">
...
<script>
    ...
    $(document).ready(() => {
        ...
        $("#btnRegist").on("click", () => {
            ...
            if(IsNull($("#frm_plan_proc_cd").val())) {
                alert("저장할 항목을 선택해주세요.");
                return false;
            }

            if(IsNull($("#frm_plan_proc_quality_cd").val())) {
                alert("측정분류를 선택해주세요.");
                return false;
            }
    ...
    function getData()
    {
        ...
        .done(function (data)
        {
            ...
            data.forEach((item, index) => {
                ...
                let firstHtml = "<div class='text-left'>" + IsEmpty(item.plan_no) + "</div>" +
                    "<input type=\"hidden\" name=\"fact_cd\" value=\"" + item.fact_cd + "\"</input>"+
                    "<input type=\"hidden\" name=\"plan_cd\" value=\"" + item.plan_cd + "\"</input>"+
                    "<input type=\"hidden\" name=\"plan_proc_cd\" value=\"" + item.plan_proc_cd + "\"</input>";
                ...
                node.push("<div class='text-center'>" + IsEmpty(item.plan_proc_quality_yn) + "</div>");
                node.push("<div class='text-center'>" + IsEmpty(item.plan_proc_quality_nm) + "</div>");
                node.push("<div class='text-center'>" + IsEmpty(item.plan_proc_quality_val) + "</div>");
    ...
    function getDataOne(plan_cd, plan_proc_cd)
    {
        ...
        $.ajax({
            url: "/mes/quality/process/processInspectOne/" + plan_cd + "/" + plan_proc_cd
            ,type: "get"
            ,dataType: "json"
            // ,data: JSON.stringify({})
        })
        .done(function (data)
        {
            ...
            $("#frm_plan_dt").text(data.plan_stdt + " ~ " + data.plan_eddt);
