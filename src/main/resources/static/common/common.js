...
function setDataOne(prefix, data) {
    $.each(data, function (key, value) {
        ...
        if (inputType === 'text' || inputType === 'hidden') {
            if (
                key.indexOf('_price') != -1 ||
                key.indexOf('_cnt') != -1 ||
                key.indexOf('_amt') != -1 ||
                key.indexOf('_total') != -1 ||
                key.indexOf('_vat') != -1
            ) {
        ...
        if (checkImageType(fileName)) {
            if (isThumb) {
                ...
                return (
                    ...
                    cust_cd +
                    '&fileName=' +
                    encodeURIComponent(getFileLink(fileName))
                );
            }
        } else {
...
function setFileEvent(gubn, cust_cd, fileName) {
    if (checkImageType(fileName)) {
        ...
        self.location =
            '/common/fileDisplay?gubn=' + gubn + '&cust_cd=' + cust_cd + '&fileName=' + encodeURIComponent(fileName);
    }
}
...
function setDownload(gubn, cust_cd, fileName) {
    ...
    if (checkImageType(fileName)) {
        ...
        self.location =
            '/common/fileDownload?gubn=' + gubn + '&cust_cd=' + cust_cd + '&fileName=' + encodeURIComponent(fileName);
    }
}
...
function markFileSize(file_size) {
    var byteSize = Math.round((file_size / 1024) * 100) * 0.01;
    var suffix = 'KB';
...
function limitFileUpload(originSize, limitSize) {
    ...
    var convertLimit = limitSize * 1024 * 0.001;
        ...
        onrendered: function (canvas) {
            ...
            for (var i = 1; i <= pdf_pages; i++) {
                ...
                pdf.addImage(
                    ...
                    'JPG',
                    margin_top_left,
                    -(pdf_height * i) + margin_top_left * 4,
                    img_width,
                    img_height,
                );
            }
    ...
    while (ss_day.getTime() <= ee_day.getTime()) {
        ...
        res_day.push(
            ss_day.getFullYear() +
                ...
                _day_ +
                '(' +
                week[new Date(ss_day.getFullYear() + '-' + _mon_ + '-' + _day_).getDay()] +
                ')',
        );
        ss_day.setDate(ss_day.getDate() + 1);
    }
